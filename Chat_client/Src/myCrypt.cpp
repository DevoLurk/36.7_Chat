...
void my_add_salt(std::string& pass, std::string& salt, const int& min_size)
{
    if (pass.length() < min_size) // append pass to min_size letters from salt string
    {
        int dif = min_size - pass.length();
        salt.assign(salt, 5, dif);
        pass.append(salt);
    }
}
